	...
	namespace server {
		...
		void CWebServer::HandleCommand(const std::string &cparam, WebEmSession & session, const request& req, Json::Value &root)
		{
			...
			else if (cparam == "switchlight")
			{
				...
				_log.Log(LOG_ERROR, "User: %s initiated a switch command (%s/%s/%s)", Username.c_str(), idx.c_str(), sSwitchName.c_str(), switchcmd.c_str());
			...
			{
				//TODO: Make compatible with new structure..
				std::string idx = request::findValue(&req, "idx");
				...
				if (iswhite != "true")
				{
					...
					int ival;
					ival = round(dval);
					m_mainworker.SwitchLight(ID, "Set Color", ival, NoColor, false, 0);
				}
				else
				{
				...
				sleep_milliseconds(100);
				m_mainworker.SwitchLight(ID, "Set Brightness", (unsigned char)atoi(brightness.c_str()), NoColor, false, 0);
				root["status"] = "OK";
				root["title"] = "SetColBrightnessValue";
			}
			else if (cparam == "setcolbrightnessvalue2")
			{
				std::string idx = request::findValue(&req, "idx");
				...
				_log.Log(LOG_ERROR, "setcolbrightnessvalue2: hex: '%s', bri: '%s', iswhite: '%s'", hex.c_str(), brightness.c_str(), iswhite.c_str());
				if (!hex.empty())
				{
					uint64_t ihex = hexstrtoui64(hex);
					_log.Log(LOG_ERROR, "setcolbrightnessvalue2: hex: '%s', ihex: %" PRIx64 ", bri: '%s', iswhite: '%s'", hex.c_str(), ihex, brightness.c_str(), iswhite.c_str());
					uint8_t t = 0;
					uint8_t r = 0;
					uint8_t g = 0;
					uint8_t b = 0;
					...
					switch (hex.length())
					{
						...
						case 10: //RGB_CW_WW
							...
							break;
						case 12: //T_RGB_CW_WW
							t = (uint8_t)((ihex & 0xFF0000000000) >> 40);
							r = (uint8_t)((ihex & 0x00FF00000000) >> 32);
							g = (uint8_t)((ihex & 0x0000FF000000) >> 24);
							b = (uint8_t)((ihex & 0x000000FF0000) >> 16);
							cw = (uint8_t)((ihex & 0x00000000FF00) >> 8);
							ww = (uint8_t)ihex & 0xFF;
							break;
					}
					color = _tColor(r, g, b, cw, ww, 0);
					color.t = t/255.0f;
					_log.Log(LOG_ERROR, "setcolbrightnessvalue2: trgbww: %02x%02x%02x%02x%02x%02x, color: '%s'", t, r, g, b, cw, ww, color.toJSONfixedpoint().c_str());
				}
				if (hex.empty() || brightness.empty() || iswhite.empty())
				{
					return;
				}
				...
				{
					color.white = true;
					m_mainworker.SwitchLight(ID, "Set Kelvin Level", -1, color, false, 0);
				}
				sleep_milliseconds(100);
				m_mainworker.SwitchLight(ID, "Set Brightness", (unsigned char)atoi(brightness.c_str()), NoColor, false, 0);
				root["status"] = "OK";
			...
			else if (cparam.find("setkelvinlevel") == 0)
			{
				...
				_tColor color = _tColor(ival/100.0f);
				_log.Log(LOG_ERROR, "setkelvinlevel: t: %d, color: '%s'", ival, color.toJSONfixedpoint().c_str());
				...
				for (itt = result.begin(); itt != result.end(); ++itt)
				{
						...
						)
					{
						...
						if (switchtype == STYPE_Dimmer)
						{
							...
							{
								_tColor color(sColor, false); // false: json string with colors as floats
								std::string jsonColor = color.toJSONfixedpoint();
								_log.Log(LOG_ERROR, "GetJSonDevices: hex: '%s', sColor: '%s'", jsonColor.c_str(), sColor.c_str());
