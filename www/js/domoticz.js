...
function ShowMonthReportTemp(actMonth, actYear) {
	$.getJSON("json.htm?type=graph&sensor=temp&idx=" + $.devIdx + "&range=year&actmonth=" + actMonth + "&actyear=" + actYear,
		function (data) {
			var lastTotal = -1;
			var bHumOnly = ((typeof data.result[0].ta == 'undefined') && (typeof data.result[0].hu != 'undefined'));
			...
			if (!bHumOnly)
				...
				htmlcontent += $('#reportviewhumiditymonth').html();
			...
			$.UsageChart.highcharts({
				...
				yAxis: {
					maxPadding: 0.2,
					title: {
						text: (!bHumOnly) ? $.t('Temperature') : $.t('Humidity')
					}
				},
				tooltip: {
					formatter: function () {
						...
						return $.t(Highcharts.dateFormat('%A', this.x)) + ' ' + Highcharts.dateFormat('%B %d', this.x) + '<br/>' + this.series.name + ': ' + this.y + '%';
					}
				},
				plotOptions: {
					column: {
			...
			var series = $.UsageChart.highcharts().get('mchart');
		...
		function (data) {
			...
			if (data.result.length > 0) {
				...
				if (!bHumOnly) {
					...
					htmlcontent += $('#reportviewhumidity').html();
				}
				...
				$.UsageChart.highcharts({
					...
					yAxis: {
						maxPadding: 0.2,
						title: {
							text: (bHumOnly == false) ? $.t('Temperature') : $.t('Humidity')
						}
					},
					tooltip: {
						formatter: function () {
							...
							return $.t(Highcharts.dateFormat('%B', this.x)) + '<br/>' + this.series.name + ': ' + this.y + '%';
						}
					},
					plotOptions: {
						column: {
			...
			$.each(data.result, function (i, item) {
				...
				if (year == actYear) {
					...
					if (!bHumOnly) {
						...
						//Humdity
						var mvalue = parseFloat(item.hu);
						total += parseFloat(item.hu);
						monthvalues++;
			...
			hlstring = "-";
			if ((!bHumOnly) && (lowest_month_val != -1)) {
			...
			var series = $.UsageChart.highcharts().get('tserie');
			series.setData(datachart);
...
function ShowTempLog(contentdiv, backfunction, id, name, devtype) {
	clearInterval($.myglobals.refreshTimer);
	$(window).scrollTop(0);
	$('#modal').show();
	$.content = contentdiv;
	...
	var TitleStart = $.t('Temperature');
	if (devtype=="Humidity") {
		TitleStart = $.t('Humidity');
	}
	...
	$.DayChart.highcharts({
			...
			events: {
				load: function () {
					$.getJSON("json.htm?type=graph&sensor=temp&idx=" + id + "&range=day", function (data) {
						...
						$.DayChart.highcharts().yAxis[1].visibility = false;
					});
				}
			}
		},
		...
		title: {
			text: TitleStart + ' ' + Get5MinuteHistoryDaysGraphTitle()
		},
		xAxis: {
			type: 'datetime'
		},
...
function ShowRGBWPopupInt(mouseX, mouseY, idx, refreshfunction, Protected, MaxDimLevel, LevelInt, colorJSON, SubType) {
	...
	{
		console.log('UpdateColorPicker: ' + mode);
		colorPickerMode = mode;
		if (mode == "color") {
			$('#rgbw_popup #popup_picker').wheelColorPicker('setOptions', {sliders:'wm', preserveWheel:true});
		}
		else if (mode == "white") {
			$('#rgbw_popup #popup_picker').wheelColorPicker('setOptions', {sliders:'m', preserveWheel:true});
		}
		else if (mode == "temperature") {
			$('#rgbw_popup #popup_picker').wheelColorPicker('setOptions', {sliders:'xm'});
		}
		else if (mode == "customw") {
			$('#rgbw_popup #popup_picker').wheelColorPicker('setOptions', {sliders:'wvlm', preserveWheel:false});
		}
		else if (mode == "customww") {
			$('#rgbw_popup #popup_picker').wheelColorPicker('setOptions', {sliders:'wvklm', preserveWheel:false});
		}
		$('#rgbw_popup #popup_picker').wheelColorPicker('refreshWidget');
		$('#rgbw_popup #popup_picker').wheelColorPicker('updateSliders');
		$('#rgbw_popup #popup_picker').wheelColorPicker('redrawSliders');
	}
	
	$.devIdx = idx;
	$('#rgbw_popup .pickermodergb').hide();
	$('#rgbw_popup .pickermodewhite').hide();
	$('#rgbw_popup .pickermodetemp').hide();
	$('#rgbw_popup .pickermodecustomw').hide();
	$('#rgbw_popup .pickermodecustomww').hide();
	if (SubType.indexOf("RGB") >= 0) {
		$('#rgbw_popup .pickermodergb').show();
	}
	if (SubType.indexOf("RGBW") >= 0 && SubType.indexOf("WW") < 0) {
		$('#rgbw_popup .pickermodewhite').show();
	}
	if (SubType.indexOf("WW") >= 0) {
		$('#rgbw_popup .pickermodetemp').show();
	}
	if (SubType.endsWith("BWZ")) {
		$('#rgbw_popup .pickermodecustomw').show();
	}
	if (SubType.endsWith("BWWZ")) {
		$('#rgbw_popup .pickermodecustomww').show();
	}
	...
	$('#rgbw_popup #popup_switch_rgb').click(function(){
		UpdateColorPicker("color");
	});
	$('#rgbw_popup #popup_switch_white').click(function(){
		UpdateColorPicker("white");
	});
	$('#rgbw_popup #popup_switch_temp').click(function(){
		UpdateColorPicker("temperature");
	});
	$('#rgbw_popup #popup_switch_customw').click(function(){
		UpdateColorPicker("customw");
	});
	$('#rgbw_popup #popup_switch_customww').click(function(){
	...
	/*$('#rgbw_popup #slider').slider({
		...
		slide: function (event, ui) { //When the slider is sliding
			...
			var fPercentage = 0;
			if (ui.value != 1) {
				fPercentage = parseInt((100.0 / maxValue) * (ui.value ));
			}
	...
	// Setup color picker
	/*if(color.match(/^[0-9a-f]{12}$/i) != null) {
		let whitelevel = 0;
		let temperature = 0;
		let cw = 0;
		let ww = 0;
		// Set RGB
		$('#rgbw_popup #popup_picker').wheelColorPicker('setValue', color.substr(2,6), true);
		
		// Set temperature and white level
		temperature = parseInt(color.substr(0, 2), 16) / 255;
		cw = parseInt(color.substr(8, 2), 16) / 255;
		ww = parseInt(color.substr(10, 2), 16) / 255;
		whitelevel = cw + ww;
		$('#rgbw_popup #popup_picker').wheelColorPicker('setTemperature', temperature, true);
		//$('#rgbw_popup #popup_picker').wheelColorPicker('setTemperature', 0.33, true);
		$('#rgbw_popup #popup_picker').wheelColorPicker('setWhite', whitelevel, true);
		//$('#rgbw_popup #popup_picker').wheelColorPicker('setWhite', 0.66, true);
	}*/
	let color_t = (color.t==null)?0.5:color.t;
	let color_cw = (color.cw==null)?0.5:color.cw;
	let color_ww = (color.ww==null)?0.5:color.ww;
	let color_r = (color.r==null)?0:color.r;
	let color_g = (color.g==null)?0:color.g;
	let color_b = (color.b==null)?0:color.b;
	
	$('#rgbw_popup #popup_picker').wheelColorPicker('setTemperature', color_t/255);
	$('#rgbw_popup #popup_picker').wheelColorPicker('setWhite', color_cw/255+color_ww/255);
	$('#rgbw_popup #popup_picker').wheelColorPicker('setRgb', color_r/255, color_g/255, color_b/255);
	
	$('#rgbw_popup #popup_picker').off('slidermove sliderup').on('slidermove sliderup', function() {
		clearInterval($.setColValue);
		//let hsv = $(this).wheelColorPicker('getValue', 'hsv');
		var bIsWhite = false;//(hsv.s < 0.1);
		let white_value = 75;
		//let dimlevel = $('#rgbw_popup #slider').slider("value");
		//var white_value = $('#rgbw_popup #white_slider').slider("option", "value") - 1;
		...
		if (colorPickerMode == "color") {
			$(this).wheelColorPicker('setHsv', color.h, color.s, 1); // Set brightness to max
			rgbhex = $(this).wheelColorPicker('getValue', 'hex');
			bIsWhite = (color.s < 0.1);
		}
		if (colorPickerMode == "temperature" || colorPickerMode == "white") {
			cwhex = Math.round((1-color.t) * 255).toString(16);
			if( cwhex.length == 1) {
				cwhex = "0" + cwhex;
			}
			wwhex = Math.round(color.t * 255).toString(16);
			if( wwhex.length == 1) {
				wwhex = "0" + wwhex;
			}
			thex = Math.round(color.t * 255).toString(16);
			if( thex.length == 1) {
				thex = "0" + thex;
			}
			var bIsWhite = true;
		}
		if (colorPickerMode == "customw" || colorPickerMode == "customww") {
			rgbhex = $(this).wheelColorPicker('getValue', 'hex');
			if (colorPickerMode == "customw") $(this).wheelColorPicker('setTemperature', 1); // Set color temperature to max
			cwhex = Math.round(color.w * (1-color.t) * 255).toString(16);
			if( cwhex.length == 1) {
				cwhex = "0" + cwhex;
			}
			wwhex = Math.round(color.w * color.t * 255).toString(16);
			if( wwhex.length == 1) {
				wwhex = "0" + wwhex;
			}
			thex = Math.round(color.t * 255).toString(16);
		...
		//let kelvin = $(this).wheelColorPicker.color.k;
		//let whitelevel = $(this).wheelColorPicker.color.l;
		
		//if (white_value<0) white_value=0;
		//if (white_value>255) white_value=255;
		console.log(thex+rgbhex+cwhex+wwhex);
		//if (colorPickerMode == "color" || colorPickerMode == "customww") {
			$.setColValue = setInterval(function () { SetColValue($.devIdx, thex+rgbhex+cwhex+wwhex, dimlevel, bIsWhite); }, 400);
		//}
		//if (colorPickerMode == "temperature") {
		//	appLampSetKelvin(color.t*100);
		//}
	});
